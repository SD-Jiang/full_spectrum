...
#points=vtkPoints()
p_file=np.loadtxt(args.p, usecols=(0,1,2,6), skiprows=1,delimiter=',')

# if p_file.shape[0]==4 and p_file.ndim==1:
#     p_file=p_file.T
# print()
...
if p_file.ndim == 1:
    ...
    source.SetCenter(p_file[0],p_file[1],p_file[2])
    ...
    actor1.GetProperty().SetColor(colors.GetColor3d(colordict[int(p_file[3]%15)][0]))
...
renderWindow.Render()
renderWindow.SetWindowName(os.path.split(args.p)[-1])
...
writer=vtkPNGWriter()
topath='./soma_figs'
tofile=os.path.join(topath,os.path.split(args.p)[-1]+'.png')
